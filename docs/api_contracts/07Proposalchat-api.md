...
Both ids are required. They are treated as opaque references into the User Request context; ProposalChat resolves them through its ACL and never persists them.
...
**Ephemeral by design.** Suggestions are never persisted: no GET by id, no list, no history. Only the `intendedUse` a human accepts — written onto the proposal in the User Request context — is durable. Re-running triage on the same message always recomputes; the endpoint is safe to call repeatedly.

**Advisory, not a command.** A suggestion never changes the proposal and never emits a `ProposalEvent`. It leaves no trace in the proposal's audit trail. Traceability starts only when a staff member applies the value through the User Request action `PUT /proposals/{proposalId}/intended-use`, which records `INTENDED_USE_UPDATED` there — not in ProposalChat.

**Reads through an ACL.** ProposalChat does not own `Conversation` or `Message`. It reads them from the User Request context through a translation layer and treats `conversationId` / `messageId` as opaque references, resolving the focus message via the conversation root. It never writes back across the boundary.
