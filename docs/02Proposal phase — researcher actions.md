...
**A proposal is always created object-free.** The collection objects a researcher wants are described in prose in the initial message; they are attached as structured `RequestedObject` entries only later, once the researcher has searched the catalog and selected the matches, via `POST /proposals/{proposalId}/requested-objects`. This endpoint accepts no `requestedObjects` field — any such field in the body is ignored.
...
**Response `415 Unsupported Media Type`** — the filename is not `.docx`, or the
bytes are not a real DOCX (a ZIP carrying the OOXML `[Content_Types].xml` part);
the content is validated, not just the extension.
```json
{
